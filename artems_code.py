...
def iterative_query(domain_name: str) -> list[str]:
    ...
    dns_query_a, transaction_id = create_dns_query(domain_name, query_type=1)
    dns_query_aaaa, _ = create_dns_query(domain_name, query_type=28)
    all_ips = []
...
def extract_records(count: int, offset: int, response: bytes) -> tuple[list, int]:
    ...
    for _ in range(count):
        ...
        if rtype == 1:
            ...
            ipv6 = ":".join(
                f"{rdata[i]:02x}{rdata[i + 1]:02x}" for i in range(0, rdlength, 2)
            )
...
def build_response(data: bytes, ip_addresses) -> bytes:
    ...
    response += b"\x00\x01"
    response += struct.pack("!H", len(ip_addresses))
...
def create_dns_query(domain: str, query_type: int) -> tuple[bytes, int]:
    ...
    query_name = (
        b"".join(struct.pack("B", len(part)) + part.encode() for part in domain_parts)
        + b"\x00"
    )
    query_class = 1
...
def run_server():
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as server:
        ...
        while True:
            ...
            ipv4_and_ipv6 = [
                (ip, 1) for ip in iterative_query(domain_name=domain) if ":" not in ip
