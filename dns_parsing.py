...
def create_dns_request(domain):
    ...
    question = (
        b"".join(
            struct.pack("B", len(part)) + part.encode("utf-8")
...
def parse_dns_query(data):
    ...
    while length != 0:
        offset += 1
        part = data[offset : offset + length].decode()
        domain.append(part)
        offset += length
        length = data[offset]
...
class DNSParser:
    ...
    def parse_dns_response(data):
        transaction_id, flags, qd_count, an_count, ns_count, ar_count, offset = (
            DNSParser._parse_header(data)
        )
    ...
    def _parse_header(data):
        try:
            transaction_id, flags, qd_count, an_count, ns_count, ar_count = (
